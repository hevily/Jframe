 ...
 *  	} );
    ...
    FixedColumns = function (oDT, oInit) {
        ...
        this.s = {
             ...
             *  @readonly
             */
            "bOldIE": (FixedColumns.isIE6 || FixedColumns.isIE7)
        };
    ...
    FixedColumns.ua = navigator.userAgent.toLowerCase();
    FixedColumns.isIE = FixedColumns.ua.indexOf('msie') > -1;
    FixedColumns.isIE6 = FixedColumns.ua.indexOf('msie 6') > -1;
    FixedColumns.isIE7 = FixedColumns.ua.indexOf('msie 7') > -1;
    FixedColumns.isIE8 = FixedColumns.ua.indexOf('msie 8') > -1;
    FixedColumns.isMozilla = FixedColumns.ua.indexOf('firefox') > -1;
    FixedColumns.isOpera = FixedColumns.ua.indexOf('opera') > -1;
	 ...
	 *  	} );
        ...
        "fnSetRowHeight": function (nTarget, iHeight) {
            ...
            if (FixedColumns.isMozilla || FixedColumns.isOpera) {
        ...
        "fnRefresh": function (reHeight) {
            if ($(this.dom.grid.dt).is(':hidden')) {
                return;
            }
            var that = this
            var oGrid = this.dom.grid,
                w = $(this.dom.grid.left.head.firstChild).outerWidth();
            oGrid.dt.style.width = $(oGrid.wrapper).width() - w + "px";
            oGrid.dt.style.left = w + 'px';
            //console.log($(this.dom.body).children("tbody").children("tr"))
            //this._fnEqualiseHeights('thead', this.dom.header, this.dom.clone.left.header, true);
            $(window).trigger("scroll." + this.s.dt.sTableId)
            this.s.dt.oInstance.fnAdjustColumnSizing(false)
            this._fnEqualiseHeights('tbody', this.dom.body, this.dom.clone.left.body, true, reHeight);
            this._fnGridHeight()
        },
        ...
        "_fnConstruct": function (oInit) {
            ...
            // $('th', this.s.dt.nTHead).each( function (i) {
            // 	iWidth = $(this).outerWidth();
            // 	that.s.aiWidths.push($(this).width());
            // 	if ( i < that.s.iLeftColumns )
            // 	{
            // 		console.log(this);
            // 		iLeftWidth += iWidth;
            // 	}
            // 	/*hide_right_columns if ( that.s.iTableColumns-that.s.iRightColumns <= i )
            // 	{
            // 		iRightWidth += iWidth;
            // 	}*/
            // } );
            ...
            /*hide_right_columns if ( this.s.iRightWidth === null )
             {
            ...
            $(this.dom.scroller).scroll(function () {
                ...
                /*hide_right_columns if ( that.s.iRightColumns > 0 )
                 {
                 that.dom.grid.right.body.scrollTop = that.dom.scroller.scrollTop;
                 }*/
            });
            $(window).on("resize.fixColumns." + this.s.dt.sTableId, function () {
                if (Pzoom.Global.isChangeWidth) {
                    if ($(that.dom.grid.left.head.parentNode).closest(':hidden').length === 1) {
                        return;
                    }

                    var oGrid = that.dom.grid,
                        w = $(that.dom.grid.left.head.firstChild).outerWidth();
                    oGrid.dt.style.width = $(oGrid.wrapper).width() - w + "px";
                    oGrid.dt.style.left = w + 'px';
                }
            });
            ...
            this.s.dt.aoDrawCallback = [
                {
                    "fn": function (o, json) {
                        that._fnDraw.call(that, bFirstDraw, json);
                        that._fnGridHeight(that);
                        bFirstDraw = false;
                        that._fnEqualiseHeights('tbody', that.dom.body, that.dom.clone.left.body);
                    },
                    "sName": "FixedColumns"
                }
            ].concat(this.s.dt.aoDrawCallback);

            this.s.dt.aoCustomCallback.push({
                "fn": function () {
                    // that._fnDraw.call( that, bFirstDraw );
                    that._fnGridHeight();
                    bFirstDraw = false;
                },
                "sName": "fixColumnsResetHeight"
            });
            ...
            bFirstDraw = false;
            // this._fnGridHeight();
            //this.s.dt.oInstance.fnDraw(false, false, false);
        },
        ...
        "_fnGridSetup": function () {
            ...
            var nSWrapper =
                $('<div class="DTFC_ScrollWrapper" style="position:relative; clear:both;">' +
                    ...
                    '<div class="DTFC_LeftHeadWrapper" style="position:relative; top:0; left:0; "></div>' +
                    '<div class="DTFC_LeftBodyWrapper" style="position:relative; top:0; left:0; "></div>' +
                    '<div class="DTFC_LeftFootWrapper" style="position:relative; top:0; left:0; overflow:hidden;"></div>' +
                    '</div>' +
                    /*hide_right_columns '<div class="DTFC_RightWrapper" style="position:absolute; top:0; left:0;">'+
            ...
            /*hide_right_columns if ( this.s.iRightColumns > 0 )
             {
            ...
            $(nSWrapper).prepend(this.dom.grid.dt);


            this.dom.grid.dt.style.position = "absolute";
            this.dom.grid.dt.style.top = "0px";
            // this.dom.grid.dt.style.left = this.s.iLeftWidth+"px";
            // this.dom.grid.dt.style.width = ($(this.dom.grid.dt).width()-this.s.iLeftWidth/*hide_right_columns -this.s.iRightWidth*/)+"px";
        },
        ...
        "_fnGridLayout": function () {
            var oGrid = this.dom.grid;
            oGrid.left.wrapper.style.width = 0;
            // oGrid.dt.style.width = $(oGrid.wrapper).width()+"px";
            // oGrid.dt.style.left = 0;
            return;
            ...
            /*hide_right_columns if ( this.s.sRightWidth == 'fixed' )
             {
            ...
            iRemainder = iTotal - iLeft/*hide_right_columns  - iRight*/;

            oGrid.left.wrapper.style.width = 0 + "px";
        ...
        "_fnGridHeight": function () {
            ...
            var tbody = $('.dataTables_scrollBody', this.dom.grid.dt);
            var isOverHeight = tbody.height() > tbody.find('> table').height();
            var that = this;
            if (!this.s.dt.nTable.isOverflow && isOverHeight) {
                iHeight -= this.s.dt.oScroll.iBarWidth;
            } else if (tbody.next('.scrollBody_placeholder').length > 0 && this.s.dt.nTable.isOverflow && !isOverHeight) {
                iHeight += this.s.dt.oScroll.iBarWidth;
            }
         ...
         *  @private
         */
        "_fnDraw": function (bAll, json) {
            this._fnCloneLeft(bAll);
            /*hide_right_columns this._fnCloneRight( bAll );*/

            /* Draw callback function */
            if (this.s.fnDrawCallback !== null) {
                this.s.fnDrawCallback.call(this, this.dom.clone.left, this.dom.clone.right, json);
            }

            /* Event triggering */
            $(this).trigger('draw', {
                "leftClone": this.dom.clone.left/*hide_right_columns,
        ...
        "_fnCloneLeft": function (bAll) {
            ...
            var that = this,
                i, jq,
                aiColumns = [],
                bodyHd = $(this.dom.body).find('> thead > tr');

            for (i = 0; i < this.s.iLeftColumns; i++) {
                if (!this.s.binded) {
                    bodyHd.find('th:first-child').remove();
                }
                aiColumns.push(i);
            }
            this.s.binded = true;
            ...
            var oGrid = this.dom.grid,
                w = $(this.dom.grid.left.head.firstChild).outerWidth();
            oGrid.left.wrapper.style.width = w + 'px';
            oGrid.dt.style.width = $(oGrid.wrapper).width() - w + "px";
            oGrid.dt.style.left = w + 'px';
...
//            }
            ...
            oClone.body = $(this.dom.body.outerHTML)[0]
            ...
            for (iIndex = 0; iIndex < aiColumns.length; iIndex++) {
                ...
                oStyle.height = 0;
                that.s.aiWidths[iColumn] && (oStyle.width = that.s.aiWidths[iColumn] + "px");
            ...
            if (this.s.dt.aiDisplay.length > 0) {
                ...
                $('>tbody>tr', that.dom.body).each(function (z) {
                    this.id = that.s.dt.sTableId + "_rtr_" + z
                    ...
                    n.id = that.s.dt.sTableId + "_ltr_" + z
                    nBody.appendChild(n);
                });
            }
            else {
            ...
            $(anUnique).each(function (i) {
                iColumn = aiColumns[i];
                that.s.aiWidths[iColumn] && (this.style.width = that.s.aiWidths[iColumn] + "px");
            });
         ...
         *  @private
         */
        "_fnEqualiseHeights": function (nodeName, original, clone, fixBug, reHeight) {
            /*if (nodeName == 'thead' || nodeName == 'tfoot') {
             return;
             }*/
            var that = this,
                i, iLen, iHeight, iHeightOriginal, iHeightClone,
                rootOriginal = original.getElementsByTagName(nodeName)[0],
                rootClone = clone.getElementsByTagName(nodeName)[0],
                anOriginal = this._fnGetTrNodes(rootOriginal),
                anClone = this._fnGetTrNodes(rootClone);
            if (FixedColumns.isIE) {
                var jqBoxHack = $('>' + nodeName + '>tr:eq(0)', original).children(':first'),
                    iBoxHack = jqBoxHack.outerHeight() - jqBoxHack.height()
            }
            if (fixBug) {
                that.dom.clone.left.body.offsetParent.parentNode.style.width = that.dom.scroller.parentNode.style.left
                that.dom.clone.left.body.offsetParent.parentNode.childNodes[0].width = that.dom.scroller.parentNode.style.left
            }
            if (fixBug || reHeight) {
                $(original).find(">tbody>tr").css("height", "")
                $(clone).find(">tbody>tr").css("height", "")
            }
            for (var i = 0, iLen = anClone.length; i < iLen; i++) {
                if (!($(anOriginal[i]).data("h") && $(anClone[i]).data("h")) || reHeight) {
                    $(anOriginal[i]).data("h", anOriginal[i].offsetHeight)
                    $(anClone[i]).data("h", anClone[i].offsetHeight)
                }
            }
            for (i = 0, iLen = anClone.length; i < iLen; i++) {

                if (anOriginal[i].parentNode.tagName.toUpperCase() === 'THEAD') {
                    return;
                }

//                if (this.s.sHeightMatch == 'semiauto' && typeof anOriginal[i]._DTTC_iHeight != 'undefined' && anOriginal[i]._DTTC_iHeight !== null && !fixBug) {
...
//                }
                ...
                iHeightOriginal = $(anOriginal[i]).data("h");
                iHeightClone = $(anClone[i]).data("h")//.offsetHeight;
                if (iHeightOriginal == iHeightClone) {
                    continue
                }

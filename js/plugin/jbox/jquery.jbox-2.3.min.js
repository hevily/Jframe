;(function (a) {
        a.jBox = function (b, c) {
            ...
            var d = (ua.indexOf('msie') && ua.indexOf('msie 6') > -1);
            var e = a('#' + c.id);
            if (e.length > 0x0) {
            ...
            if (!f.isObject) {
                b = b + '';
                var N = b.toLowerCase();
                if (N.indexOf('id:') == 0x0) f.type = 'ID'; else if (N.indexOf('get:') == 0x0) f.type = 'GET'; else if (N.indexOf('post:') == 0x0) f.type = 'POST'; else if (N.indexOf('iframe:') == 0x0) f.type = 'IFRAME'; else if (N.indexOf('html:') == 0x0) f.type = 'HTML'; else {
                    b = 'html:' + b;
                    f.type = 'HTML'
                }
                ;
            ...
            if (c.draggable) {
                ...
                //增加外层包围 lhg
                j.push('<div class="testClass"><div id="jbox" class="jbox" style="position:relative;display:inline-block;vertical-align: middle;">');
            }
            ...
            if (g) {
                ...
                j.push('<div class="jbox-title' + (c.showIcon == true ? ' jbox-title-icon' : (c.showIcon == false ? '' : ' ' + c.showIcon)) + '" style="float:left; width:' + i + '; line-height:18px; padding-left:' + (c.showIcon ? 0x12 : 0x5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (c.title == '' ? '&nbsp;' : c.title) + '</div>');
                j.push('</div>')
            }
            ;
            ...
            var o = n.find('#jbox');
            var p = n.find('#jbox-fade');
            var q = n.find('#jbox-temp');
            if (!f.isObject) {
                switch (f.type) {
                    case "ID":
                        f.html = a('#' + b).html();
            ...
            var B = function () {
                if (c.isMessager) {
                    ...
                    c.draggable && o.css({
                        ///自适应高度 lhg
                        position: "absolute",
                        top: (c.isTip ? Number($(window).height() * .35) + l.scrollTop() : ($(window).height() - o.height()) / 2),
                ...
                if ((c.showFade && !c.isTip) || (!c.showFade && !c.isTip && !c.isMessager)) {
                    n.css({
                        position: (d) ? "absolute" : "fixed",
                        height: c.showFade ? "100%" : 0x0,//l.height() : 0x0,//lhg 修改高度
                        width: "100%",
                    ...
                    if (c.isTip) {
                        ...
                        if (tip && tip.next == true) {
                            ...
                            if (N == o.offset().top) {
                                ...
                                // 0x1f4, J)
                                /*WTS*/
                                o.find('#jbox-content').html(tip.options.content.substr(0x5)).end().hide();
                                J();
                                // o.parents('.jbox-tip').remove();
                            }
                        } else {
                            o.animate({
                                top: '-=200',
        ...
        a.jBox.stateDefaults.buttons['确定']='ok';
        a.jBox.tipDefaults = {
            content: '',
            icon: 'info',
            top: '40%',
        ...
        a.jBox.close = function (b, c) {
            ...
            if (typeof b == 'string') {
                ...
                // **wts 修改**
                fn = a('#' + b).find('.jbox-close').data('fn');
                ...
                if (b) {
                    for (var e = 0x0, l = d.length; e < l; ++e) {
                        ...
                        // **wts 修改**
                        fn = d.eq(e).find('.jbox-close').data('fn');
                        if(typeof fn ==='function'){
                            fn();
                        }
                    }
                    ...
                    if (d.length > 0x0) {
                        ...
                        // **wts 修改**
                        fn =d.eq(d.length - 0x1).find('.jbox-close').data('fn');
                        if(typeof fn ==='function'){
                            fn();
                        }
                    }
